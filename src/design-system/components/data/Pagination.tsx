...
export function Pagination({ page = 1, pageCount = 1, pageSize, total, onPageChange, ...rest }: PaginationProps) {
  const pages = [];
    ...
    <div className="qos-pagination" {...rest}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
        ...
        {pages.map((p, i) => p === "…"
          ...
          : <button key={p} type="button" className="qos-page-btn" data-selected={p === page || undefined} onClick={() => onPageChange && typeof p === "number" && onPageChange(p)}>{p}</button>)}
