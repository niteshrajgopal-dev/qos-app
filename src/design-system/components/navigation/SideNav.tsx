...
export function SideNav({ groups = [], activeId, onNavigate, brand, footer, collapsed = false, ...rest }: SideNavProps) {
          ...
          <div key={g.label || gi}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
              {g.items.map((item) => {
                ...
                const selected = activeId === item.id || (hasChildren && !isOpen && children.some((c) => c.id === activeId));
                return (
                  <div key={item.id}>
                    <button
                      type="button"
