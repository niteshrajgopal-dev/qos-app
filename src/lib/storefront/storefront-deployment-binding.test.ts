...
integrationDescribe("storefront deployment binding", () => {
  ...
  it("rejects Quotes hostname + Floréa deployment binding", async () => {
    await seedQuotesStorefront();
    const flowers = await seedFlowerStorefront();

    await upsertStorefrontDeployment(db, flowers.tenant.tenant.id, {
      storefrontPublicId: flowers.storefront.publicId,
      environment: "dev",
      region: "uaenorth",
    ...
    const restore = withBoundStorefront(flowers.storefront.publicId);
