...
import { PageHeader, Card, Button, StatusBadge, Badge, Tabs, Breadcrumbs, Icon, IconButton, Input, Select, Switch, SegmentedControl, DataTable, Timeline, Alert, Banner, Toast, ToastStack, ConfirmDialog, Drawer, Chip, Logo, ProgressBar, Tag } from "@/design-system";
...
export function ChannelsScreen() {
  ...
  return (
    <>
      ...
      />
      <Grid cols={3} style={{ marginTop: 20 }}>
  ...
  return (
    <>
      ...
      <div style={{ marginTop: 20 }}>
        {tab === "overview" ? (
          <Grid cols={3} style={{ alignItems: "start" }}>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              ...
              <Card header="Recent changes" padding="sm">
                <Timeline items={platform.RELEASES.slice(0, 3).map((r) => ({ title: r.version, meta: `${r.date} · ${r.user}`, tone: r.state === "failed" ? "error" : "success", icon: r.state === "failed" ? "x-circle" : "upload-cloud" }))} />
              </Card>
            </div>
          </Grid>
        ) : null}
