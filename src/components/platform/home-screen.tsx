...
import { Section, Grid, DefinitionList } from "@/components/platform/layout";
import { PageHeader, KpiCard, Card, Button, StatusBadge, Badge, TrendChart, SegmentedControl, IntelligenceCard, Alert, Timeline, Icon, Sparkline, Avatar, ConfirmDialog, Toast, ToastStack } from "@/design-system";
    ...
    <>
      ...
      <Grid cols={3} style={{ marginTop: 16, alignItems: "start" }}>
        <Card header="Orders and revenue" subtitle="Last 24 hours, all channels" style={{ gridColumn: "span 2" }} actions={<Badge tone="neutral">Updated 2m ago</Badge>}>
          <TrendChart
            series={[{ label: "Orders", values: platform.ORDER_TREND }, { label: "Revenue", values: platform.REVENUE_TREND }]}
        ...
        <Card header="Channel health" actions={<Button size="sm" variant="ghost" onClick={() => onNavigate("channels")}>All channels</Button>} padding="none">
          {platform.CHANNELS.slice(0, 4).map((c) => (
