...
import { PageHeader, Card, Button, SegmentedControl, TrendChart, BarChart, ChartLegend, DataTable, Badge, StatusBadge, Select, IntelligenceCard, Icon, Tabs } from "@/design-system";
...
export function AnalyticsScreen() {
  ...
  return (
    <>
      ...
      <Grid cols={4} style={{ marginTop: 20 }}>
      ...
      <Grid cols={3} style={{ marginTop: 16, alignItems: "start" }}>
        <Card header="Revenue by day" subtitle={`Last ${range}`} style={{ gridColumn: "span 2" }}>
          <TrendChart
            series={[{ label: "This period", values: platform.REVENUE_TREND }, { label: "Previous period", values: platform.REVENUE_TREND.map((v, i) => Math.round(v * (0.82 + (i % 5) * 0.03))) }]}
      ...
      <Grid cols={3} style={{ marginTop: 16, alignItems: "start" }}>
        <Card header="Top products" padding="none" style={{ gridColumn: "span 2" }}>
          <DataTable
            ...
            rows={platform.PRODUCTS.filter((p) => p.state === "published")}
          />
        </Card>
        <IntelligenceCard
          kind="Insight"
      ...
      <Grid cols={2} style={{ marginTop: 16, alignItems: "start" }}>
