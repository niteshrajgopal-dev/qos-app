...
import { PageHeader, Card, Button, DataTable, StatusBadge, Badge, Tabs, Breadcrumbs, Icon, Input, Select, Switch, Checkbox, Banner, Toast, ToastStack, Alert, TableToolbar, SearchInput, Chip } from "@/design-system";
          ...
          {tab === "integrations" ? (
            <div style={{ display: "grid", gap: 16 }}>
              ...
              <Card header="Connected at this branch" padding="none">
                {platform.INTEGRATIONS.slice(0, 4).map((i) => (
          ...
          {tab === "orders" ? (
            <Card header="Recent orders at this branch" padding="none">
              <DataTable
                ...
                rows={platform.ORDERS.filter((o) => o.location === location.name)}
              />
            </Card>
          ) : null}
        </div>
